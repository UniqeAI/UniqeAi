# -*- coding: utf-8 -*-
"""
🚀 ULTIMATE HUMAN-LEVEL DATASET GENERATOR V3 - SUPREME EDITION
=============================================================

Bu script, "olağanüstü model" hedefi için %100 Pydantic uyumlu, sıfır toleranslı,
en üst seviye kalitede veri üretecek şekilde tamamen yeniden tasarlanmıştır.

🔥 TEMEL İLKELER:
✅ %100 Pydantic Validasyon Zorunluluğu - Hiçbir istisna yok
✅ telekom_api_schema.py'ye Mutlak Uyumluluk - Tek kaynak doğrusu
...
class SupremeHumanLevelDatasetGenerator:
    ...
    def _validate_tool_call(self, function_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """
        SUPREME V3: GELİŞMİŞ TOOL CALL VALİDASYONU
        
        Bu fonksiyon, "sıfır tolerans" ilkesinin kalbidir. Her API çağrısı
        şemada tanımlı olmalı ve parametreleri uygun olmalıdır.
        
        Yeni özellikler:
        - API_MAP kontrolü
        - Response model varlığı kontrolü
        - Parametre tiplerinin temel kontrolü
        - Zorunlu parametrelerin varlığı (gelecekte eklenebilir)
        """
        try:
            # 1. Fonksiyon adının API_MAP'te olup olmadığını kontrol et
            if function_name not in API_MAP:
                self.schema_violations += 1
                return {
                    "valid": False,
                    "error": f"❌ KRİTİK ŞEMA İHLALİ: Fonksiyon '{function_name}' telekom_api_schema.py API_MAP'te tanımlı değil"
                }
            
            # 2. Response modelinin var olup olmadığını kontrol et
            if function_name not in self.api_response_map:
                self.schema_violations += 1
                return {
                    "valid": False,
            ...
            # 3. Parametrelerin temel kontrolü
            ...
            # 4. GELİŞMİŞ PARAMETRE VALİDASYONU
            # Her parametre anahtarının makul olup olmadığını kontrol et
            if parameters:
                suspicious_params = []
                for param_key in parameters.keys():
                    # Parametre adı boş, çok kısa veya geçersiz karakterler içeriyor mu?
                    if not param_key or len(param_key) < 2:
                        suspicious_params.append(f"'{param_key}' (çok kısa)")
                    elif not param_key.replace('_', '').isalnum():
                        suspicious_params.append(f"'{param_key}' (geçersiz karakter)")
                
                if suspicious_params:
                    return {
                        "valid": False,
                        "error": f"❌ PARAMETRE VALİDASYON HATASI: Şüpheli parametre isimleri: {', '.join(suspicious_params)}"
                    }
            
            # 5. TELEKOM API SPESİFİK KONTROLLER
            ...
            # 6. SUPREME V3: PARAMETRE ŞEMA UYUMLULUĞU KONTROL
    ...
    def _get_required_parameters(self, function_name: str) -> List[str]:
        ...
        user_id_functions = [
            "get_current_bill", "get_past_bills", "get_payment_history", 
            "setup_autopay", "get_customer_package", "get_remaining_quotas", 
            "change_package", "enable_roaming", "get_customer_profile", 
        ...
        # Ödeme fonksiyonları için hem user_id hem bill_id gerekli
        ...
        if function_name in user_id_functions:
            return ["user_id"]
        elif function_name in payment_functions:
            return ["user_id", "bill_id", "amount"]  # ŞEMA UYUMLU: 3 zorunlu parametre
    ...
    def _generate_basic_type_data(self, field_type: Any, field_name: str = "") -> Any:
        ...
        if field_type == str:
            ...
            # ID alanları - Gerçekçi formatlar
            if "id" in field_lower:
                if "bill" in field_lower or "fatura" in field_lower:
                    return f"F-2024-{random.randint(100000, 999999)}"
            ...
            # Durum alanları
                ...
                if "bill" in field_lower or "payment" in field_lower:
                    return random.choice(["paid", "unpaid", "overdue", "processing"])
                elif "ticket" in field_lower:
                    return random.choice(["open", "in_progress", "resolved", "closed"])
                else:
            ...
            return {"key": "value", "example": random.randint(1, 100)}
    ...
    def _create_validated_response(self, model_class, override_data: Optional[Dict] = None) -> str:
        ...
        try:
            ...
            # Pydantic modeli ile veriyi doğrula ve oluştur
    ...
    def _verify_pydantic_compliance(self, scenario: Dict[str, Any]) -> Dict[str, Any]:
        """
        SUPREME VERSİYON V3: TAM PYDANTİC MODEL UYUMLULUK KONTROL
        
        Bu fonksiyon artık her API yanıtını ilgili Pydantic modeline göre
        gerçek validasyon yapar. API çağrısı ve yanıtı arasındaki uyumu kontrol eder.
        
        Yenilikler:
        - Tool call -> Response model eşleştirmesi
        - Gerçek Pydantic model validasyonu  
        - API çağrısı ve yanıtı tutarlılık kontrolü
        """
        validated_count = 0
        tool_call_response_pairs = []  # Tool call'ları ve yanıtlarını eşleştirmek için
        
        try:
            # 1. AŞAMA: Tool call'ları topla ve doğrula
            for turn in scenario["donguler"]:
                if turn.get("arac_cagrilari"):
                    for call in turn["arac_cagrilari"]:
                        ...
                        # Tool call'ı şemaya göre doğrula
                        validation = self._validate_tool_call(function_name, parameters)
                        if not validation["valid"]:
                            return {
                                "valid": False, 
                                "error": f"Tool call hatası: {validation['error']}",
            ...
            # 2. AŞAMA: API yanıtlarını kontrol et ve Pydantic validasyonu yap
            for turn in scenario["donguler"]:
                if turn.get("rol") == "arac" and turn.get("icerik"):
                    try:
                        ...
                        if matching_call:
                            ...
                            # İlgili Pydantic modelini bul
                            if function_name in self.api_response_map:
                                response_model_class = self.api_response_map[function_name]
                                
                                try:
                                    # GERÇEKESİNDEN PYDANTİC VALİDASYONU YAP
                                    validated_instance = response_model_class(**response_data)
                                    validated_count += 1
                                    
                                    print(f"✅ Pydantic validasyon başarılı: {function_name} -> {response_model_class.__name__}")
                                    
                                except ValidationError as ve:
                                    return {
                                        "valid": False,
                                        "error": f"❌ PYDANTİC VALİDASYON HATASI: {function_name} için {response_model_class.__name__} modeli uyumsuz. Hata: {ve}",
                                        "validated_count": validated_count
                                    }
                            else:
                                return {
                                    "valid": False,
                                    "error": f"❌ Response model bulunamadı: {function_name}",
                                    "validated_count": validated_count
                                }
                        else:
                            # Tool call olmadan API yanıtı - warning verip devam et
                            validated_count += 1
                            print(f"⚠️ Uyarı: Eşleşen tool call olmadan API yanıtı bulundu")
    ...
    def generate_temporal_reasoning_scenario(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "donguler": [
                ...
                {"rol": "asistan", "icerik": None, "arac_cagrilari": [{"fonksiyon": "get_past_bills", "parametreler": {"user_id": user_id}}]},
