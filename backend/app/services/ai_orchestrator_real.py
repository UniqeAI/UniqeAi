    ...
    def _load_model(self):
        ...
        try:
            ...
            try:
                ...
                # Doğrudan dosya yolu ile yükle
                model_path = "/home/enes/.cache/huggingface/hub/models--Choyrens--ChoyrensAI-Telekom-Agent-v4-gguf/blobs/13ea9c768d3cb1d9807649979308ab84398a0652cbbfa686bbd6f47b081804a8"
                ...
                self.model = Llama(
                    ...
                    n_threads=8,  # Thread sayısını artır
                    n_batch=8,  # Batch size artır
...
- get_current_package: Mevcut paket (mevcut paketim, şu anki paketim, güncel paketim)
...
- "mevcut paketim" → get_current_package
...
Kullanıcı sorusu: """
    ...
    async def _generate_response(self, dialogue: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            logger.info("   🤖 AI kendi kararını verecek")
            ...
            if self._model_loaded and self.model:
                try:
                    ...
                    response = self.model(
                        f"""Sen Choyrens AI, samimi ve yardımsever bir Telekom müşteri hizmetleri asistanısın. Kullanıcıyla doğal bir konuşma yap ve ihtiyacını anla.

KİŞİLİĞİN:
- Samimi ve dostane bir ton kullan
- Empatik ol, kullanıcının sorununu anladığını göster
- Gereksiz teknik terimler kullanma
- Doğal Türkçe konuş, resmi olma
- Yardım etmeye odaklan

YANIT TARZIN:
- "Merhaba! Size nasıl yardımcı olabilirim?" gibi samimi
- "Anlıyorum, bu durum gerçekten sinir bozucu olabilir" gibi empatik
- "Hemen kontrol edeyim" gibi yardımsever
- "Size en uygun çözümü bulacağım" gibi güven verici

ÖRNEKLER:
- "merhaba" → "Merhaba! Ben Choyrens AI, size nasıl yardımcı olabilirim? 😊"
- "telefonum çekmiyor" → "Ah, bu gerçekten sinir bozucu bir durum! Hemen ağ durumunuzu kontrol edeyim. [check_network_status]"
- "faturamı görmek istiyorum" → "Tabii ki! Geçmiş faturalarınızı hemen getiriyorum. [get_past_bills]"
- "paketim nedir" → "Mevcut paketinizi kontrol edeyim, size detayları göstereyim. [get_current_package]"

Telekom araçları:
- get_past_bills: Geçmiş faturalar, önceki faturalar, fatura geçmişi
- get_current_bill: Mevcut fatura, şu anki fatura, güncel fatura
- get_current_package: Mevcut paket, şu anki paketim, güncel paketim
...
Asistan:""",
                    ...
                    logger.info("   🤖 AI araç seçimi yapıyor")
            ...
            return "Merhaba! Ben Choyrens AI, size nasıl yardımcı olabilirim? Fatura, paket, teknik destek veya başka bir konuda sorularınızı yanıtlayabilirim. 😊"
            
        except Exception as e:
            logger.error(f"AI yanıt üretme hatası: {e}")
            return "Üzgünüm, şu anda biraz karışık düşünüyorum. Lütfen bir dakika sonra tekrar deneyin! 😅"
