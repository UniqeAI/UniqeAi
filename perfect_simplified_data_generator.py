...
class ExtendedDataGenerator:
    ...
    def generate_analytics_reporting_data(self) -> List[Dict[str, str]]:
        """Gelişmiş analitik ve raporlama veri örnekleri"""
        data = []
        
        data.append(self.create_data_point(
            instruction="Satış raporu oluştur",
            input_text="Bu ayki toplam satış rakamlarını kategori bazında analiz et",
            output_code="print(backend_api.generate_sales_report(period='this_month'))"
        ))
        
        data.append(self.create_data_point(
            instruction="Müşteri davranış analizi",
            input_text="En çok satın alınan ürünleri ve müşteri segmentlerini analiz et",
            output_code="print(backend_api.analyze_customer_behavior())"
        ))
        
        data.append(self.create_data_point(
            instruction="Trend analizi",
            input_text="Son 6 aylık satış trendlerini ve gelecek 3 ay için tahmin yap",
            output_code="print(backend_api.generate_sales_report(months=6, forecast=3))"
        ))
        
        # YENİ PROFESYONEL ANALİTİK & RAPORLAMA (+22)
        data.append(self.create_data_point(
            instruction="AI destekli satış tahmini",
            input_text="Makine öğrenmesi ile gelecek çeyrek satış tahminlerini oluştur",
            output_code="print(backend_api.generate_sales_report(quarter='next'))"
        ))
        
        data.append(self.create_data_point(
            instruction="Müşteri yaşam döngüsü değeri",
            input_text="Her müşteri segmenti için CLV (Customer Lifetime Value) hesapla",
            output_code="print(backend_api.analyze_customer_behavior())"
        ))
        
        data.append(self.create_data_point(
            instruction="Gerçek zamanlı dashboard",
            input_text="Canlı satış, stok ve müşteri metriklerini dashboard'da göster",
            output_code="print(backend_api.generate_sales_report(real_time=True))"
        ))
        
        data.append(self.create_data_point(
            instruction="Cohort analizi",
            input_text="Müşteri kazanım cohort'larını analiz et ve retention hesapla",
            output_code="print(backend_api.analyze_customer_behavior())"
        ))
        
        data.append(self.create_data_point(
            instruction="A/B test sonuçları",
            input_text="Website ve uygulama A/B testlerinin istatistiksel analizini yap",
            output_code="print(backend_api.generate_sales_report(test_type='ab'))"
        ))
        
        data.append(self.create_data_point(
            instruction="Competitive intelligence",
            input_text="Rakip fiyatlandırma ve pazar pozisyon analizi yap",
            output_code="print(backend_api.generate_sales_report())"
        ))
        
        data.append(self.create_data_point(
            instruction="Seasonality analizi",
            input_text="Mevsimsel satış paternlerini ve özel günlerin etkisini analiz et",
            output_code="print(backend_api.generate_sales_report())"
        ))
        
        data.append(self.create_data_point(
            instruction="Attribution modeling",
            input_text="Multi-touch attribution ile pazarlama kanallarının etkisini ölç",
            output_code="print(backend_api.analyze_customer_behavior())"
        ))
        
        data.append(self.create_data_point(
            instruction="Predictive churn modeling",
            input_text="Müşteri kaybı riskini önceden tahmin et ve önleme stratejileri öner",
            output_code="print(backend_api.analyze_customer_behavior())"
        ))
        
        data.append(self.create_data_point(
            instruction="Market basket analysis",
            input_text="Ürünler arası ilişkileri analiz et ve bundle önerileri oluştur",
            output_code="print(backend_api.analyze_customer_behavior())"
        ))
        
        data.append(self.create_data_point(
            instruction="Geographic performance",
            input_text="Bölgesel satış performansını harita üzerinde görselleştir",
            output_code="print(backend_api.generate_sales_report())"
        ))
        
        data.append(self.create_data_point(
            instruction="Price elasticity analysis",
            input_text="Fiyat değişikliklerinin talep üzerindeki etkisini analiz et",
            output_code="print(backend_api.generate_sales_report())"
        ))
        
        data.append(self.create_data_point(
            instruction="Inventory turnover optimization",
            input_text="Stok devir hızını optimize etmek için analitik öneriler sun",
            output_code="print(backend_api.generate_sales_report())"
        ))
        
        data.append(self.create_data_point(
            instruction="Customer acquisition cost",
            input_text="Kanal bazında müşteri kazanım maliyetlerini hesapla ve optimize et",
            output_code="print(backend_api.analyze_customer_behavior())"
        ))
        
        data.append(self.create_data_point(
            instruction="Revenue attribution",
            input_text="Gelir kanallarını detaylı olarak analiz et ve optimize etme önerileri sun",
            output_code="print(backend_api.generate_sales_report())"
        ))
        
        data.append(self.create_data_point(
            instruction="Anomaly detection",
            input_text="Satış ve müşteri davranışlarındaki anormallikleri otomatik tespit et",
            output_code="print(backend_api.analyze_customer_behavior())"
        ))
        
        data.append(self.create_data_point(
            instruction="Social media sentiment",
            input_text="Sosyal medya mentions ve sentiment analizi ile marka algısını ölç",
            output_code="print(backend_api.analyze_customer_behavior())"
        ))
        
        data.append(self.create_data_point(
            instruction="Supply chain analytics",
            input_text="Tedarik zinciri performansını ve risk faktörlerini analiz et",
            output_code="print(backend_api.generate_sales_report())"
        ))
        
        data.append(self.create_data_point(
            instruction="Cross-sell effectiveness",
            input_text="Çapraz satış kampanyalarının etkinliğini ölç ve optimize et",
            output_code="print(backend_api.analyze_customer_behavior())"
        ))
        
        data.append(self.create_data_point(
            instruction="Dynamic pricing impact",
            input_text="Dinamik fiyatlandırmanın gelir ve kar marjı üzerindeki etkisini analiz et",
            output_code="print(backend_api.generate_sales_report())"
        ))
        
        data.append(self.create_data_point(
            instruction="Customer journey mapping",
            input_text="Müşteri yolculuğunu detaylı analiz et ve iyileştirme noktalarını belirle",
            output_code="print(backend_api.analyze_customer_behavior())"
        ))
        
        data.append(self.create_data_point(
            instruction="Profitability analysis",
            input_text="Ürün, kategori ve müşteri bazında karlılık analizini detaylandır",
            output_code="print(backend_api.generate_sales_report())"
        ))
    ...
    def generate_all_data(self) -> List[Dict[str, str]]:
        ...
        # Telekom kategorileri (5 kategori x 25 = 125 data points)
        all_data.extend(self.generate_telekom_package_data())
        all_data.extend(self.generate_telekom_billing_data())
        all_data.extend(self.generate_telekom_technical_data())
        all_data.extend(self.generate_telekom_line_data())
        all_data.extend(self.generate_telekom_internet_tv_data())
        
        # E-commerce kategorileri (7 kategori x 25 = 175 data points)
        all_data.extend(self.generate_product_management_data())
        all_data.extend(self.generate_order_management_data())
        all_data.extend(self.generate_customer_management_data())
        all_data.extend(self.generate_payment_billing_data())
        all_data.extend(self.generate_inventory_stock_data())
        all_data.extend(self.generate_support_communication_data())
        all_data.extend(self.generate_analytics_reporting_data())
        
        return all_data

    def save_to_json(self, filename: str = "synthetic_training_data.json"):
        ...
        telekom_count = 125  # 5 x 25
        ecommerce_count = 175  # 7 x 25
...
if __name__ == "__main__":
    print("🚀 Gün 2: Genişletilmiş Sentetik Veri Üretimi Başlıyor...")
    ...
    dataset = generator.save_to_json("perfect_simplified_dataset_300_points.json")
